    ...
    componentDidMount() {
        ...
        this.onAudioProcess = new RealTimeBPMAnalyzer({
            ...
            pushCallback: (err, bpm2) => {
                if (bpm2 && bpm2.length) {
                    console.log(bpm2[0]);
                    const firstThreshold = Object.keys(
                        this.onAudioProcess.validPeaks
                    )[0];
                    ...
                    while (this.indexBuffer.length >= 5) {
                        ...
                        for (let threshold of Object.keys(
                            this.onAudioProcess.validPeaks
                        )) {
                            this.onAudioProcess.validPeaks[threshold].splice(
                                0,
                                index
                            );
                            break;
                        }
                        this.indexBuffer = this.indexBuffer.map(
                            (idx) => idx - index
                        );
